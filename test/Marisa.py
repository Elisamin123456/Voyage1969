...
def draw_mist(screen):
    """
    对于不在视野内的每个格子绘制迷雾。
    如果该格子被侦察（recon_position）暴露，或被非定向激光（laser_reveal）
    或 Master Spark（masterspark_reveal）暴露，则不绘制迷雾。
    """
    ...
    for y in range(MAP_HEIGHT):
        for x in range(MAP_WIDTH):
            ...
            if "recon_position" in game_state and cell == game_state["recon_position"]:
                continue
            # 如果该格被非定向激光暴露（laser_reveal）或 Master Spark 暴露
            if ("laser_reveal" in game_state and (x == game_state["laser_reveal"][0] or y == game_state["laser_reveal"][1])) \
               or ("masterspark_reveal" in game_state and cell in game_state["masterspark_reveal"]):
                continue
...
def compute_masterspark_reveal():
    """
    根据当前 Master Spark 状态，采样激光经过的所有网格。
    激光宽度为192像素（3格），因此在采样时对中央射线左右分别采样（例如以-96到+96范围内，每隔32像素采样）。
    返回一个包含所有经过网格坐标的集合。
    """
    reveal = set()
    if "masterspark" not in game_state:
        return reveal
    ms = game_state["masterspark"]
    start = ms["start_pos"]
    ...
    # 设定采样步长
    step_d = 5  # 每5像素采样一次
    # 设定横向采样范围：从 -96 到 +96（192像素宽）
    offsets = [i for i in range(-96, 97, 32)]
    # 最大采样距离：直到超出地图边界
    max_distance = 10000
    for d in range(0, max_distance, step_d):
        pos_main = (start[0] + direction[0]*d, start[1] + direction[1]*d)
        # 若超出地图范围则退出采样
        if pos_main[0] < 0 or pos_main[0] >= MAP_WIDTH*GRID_SIZE or pos_main[1] < 0 or pos_main[1] >= MAP_HEIGHT*GRID_SIZE:
            break
        for offset in offsets:
            pos = (pos_main[0] + perp[0]*offset, pos_main[1] + perp[1]*offset)
...
def update_bullets():
    """
    更新子彈狀態：
      - 技能2的子彈具有穿透效果，每個實體僅受一次傷害判定；
      - 技能1及普通攻擊（"normal"）在遇到實體時造成傷害後消失。
      
    當牆體被擊碎（wall_health[cell] <= 0）時，
    根據該牆體的總生命值 N（記錄在 wall_total 中），
    擊碎該牆體的機體（即子彈的 "owner" 屬性所指玩家）會獲得 4 * N 金幣。
    """
    ...
    for bullet in game_state["bullets"][:]:
        ...
        if cell in map_layout["walls"]:
            # 若子彈屬於技能1或普通攻擊（"normal"）
            if bullet.get("skill") in [1, "normal"]:
                wall_health[cell] -= 1
                # 當牆體生命值耗盡時，執行獎勵邏輯
                if wall_health[cell] <= 0:
                    # 從 wall_total 中取得牆體的總生命值 N（默認為 5）
                    N = wall_total.get(cell, 5)
                    # 根據子彈的 "owner" 屬性給予該玩家 4N 金幣（避免硬編碼 P1/P2）
                    owner = bullet.get("owner")
                    if owner:
                        PLAYER_STATS[owner]["gold"] += 4 * N  # 添加金幣獎勵
                    # 移除被摧毀的牆體
        ...
        # 檢查是否撞到敵方（假設敵方位置為 map_layout["start_positions"][1]）
        if cell == map_layout["start_positions"][1]:
            ...
            # 若子彈屬於技能2（穿透效果）
                ...
                if cell not in bullet["hit_entities"]:
                    PLAYER_STATS["P2"]["hp"] -= 1
                    PLAYER_STATS[MANUAL_PLAYER]["mana"] += 1  # 命中敵方返還 1 靈力
                    bullet["hit_entities"].add(cell)
                # 技能2子彈撞敵後不消失
...
def update_masterspark():
    if "masterspark" not in game_state:
        return
    ms = game_state["masterspark"]
    current_turn = game_state["current_turn"]["turn_number"]
    if current_turn > ms["activation_turn"]:
        # 新回合开始，应用一次伤害
        apply_masterspark_damage()
        ms["remaining_rounds"] -= 1
        ms["activation_turn"] = current_turn
        if ms["remaining_rounds"] <= 0:
            del game_state["masterspark"]
            if "masterspark_immobile" in game_state:
                del game_state["masterspark_immobile"]
            if "masterspark_reveal" in game_state:
                del game_state["masterspark_reveal"]
...
def get_cells_along_ray(start, direction):
    """
    从 start 出发，沿 direction 每像素采样直到超出地图边界，
    返回射线经过的所有网格坐标（元组形式）。
    """
    cells = set()
    x, y = start
    max_distance = 10000  # 足够大的数值
    for d in range(0, max_distance):
        pos_x = x + direction[0] * d
        pos_y = y + direction[1] * d
        # 若超出地图范围则退出
        if pos_x < 0 or pos_x >= MAP_WIDTH * GRID_SIZE or pos_y < 0 or pos_y >= MAP_HEIGHT * GRID_SIZE:
            break
        cell = (int(pos_x) // GRID_SIZE, int(pos_y) // GRID_SIZE)
        cells.add(cell)
    return cells

def apply_masterspark_damage():
    import math
    if "masterspark_reveal" not in game_state:
        return
    ms_reveal = game_state["masterspark_reveal"]
    current_player = game_state["current_turn"]["active_player"]
    # 计算伤害：对于玩家，根据各自max_hp；对于墙体，根据wall_total
    # 遍历所有被激光经过的网格
    for cell in ms_reveal:
        # 墙体处理
        if cell in map_layout["walls"]:
            dmg = math.ceil(wall_total.get(cell, 5) * 0.5)
            wall_health[cell] -= dmg
            if wall_health[cell] <= 0:
                PLAYER_STATS[current_player]["gold"] += 4 * wall_total.get(cell, 5)
                map_layout["walls"].remove(cell)
        # 敌方处理：确定敌方机体所在格
        ...
        if cell == enemy_cell:
            dmg = math.ceil(PLAYER_STATS[enemy_id]["max_hp"] * 0.5)
            PLAYER_STATS[enemy_id]["hp"] -= dmg
...
def draw_masterspark_indicator(screen):
    """
    绘制 Master Spark 指示器：
      - 从魔理沙中心出发，向鼠标方向延伸无限长的主射线
      - 同时绘制两条平行射线，分别偏移+64像素和-64像素（垂直于射线）
    采用预先放大4倍的采样图绘制（48×1→192×4），使用类笔刷的连续铺贴方式
    """
    import math
    if game_state.get("selected_skill") != 4 or not game_state.get("aiming", False):
        return
    # 获取魔理沙中心
    current_player = game_state["current_turn"]["active_player"]
    marisa_pos = map_layout["start_positions"][CONTROLLED_INDEX]
    marisa_center = (marisa_pos[0] * GRID_SIZE + GRID_SIZE // 2, marisa_pos[1] * GRID_SIZE + GRID_SIZE // 2)
    # 计算从魔理沙中心到鼠标方向
    mouse_pos = pygame.mouse.get_pos()
    dx = mouse_pos[0] - marisa_center[0]
    dy = mouse_pos[1] - marisa_center[1]
    ...
    # 加载并放大采样图：原始48×1放大4倍得到192×4
    sample_img = load_scaled_image("sample/skill/marisa/masterspark.png", (192, 4))
    rotated_img = pygame.transform.rotate(sample_img, angle)
    # 计算垂直方向单位向量
    perp = (-direction[1], direction[0])
    # 确定射线绘制长度（例如至窗口边界）
    max_length = 10000
    # 为保证连续性，每1像素步长绘制（可根据性能适当调整为2像素）
    for d in range(0, max_length, 4):
        pos_main = (marisa_center[0] + direction[0]*d, marisa_center[1] + direction[1]*d)
        # 若超出窗口则结束
        if pos_main[0] < 0 or pos_main[0] >= WINDOW_WIDTH or pos_main[1] < 0 or pos_main[1] >= WINDOW_HEIGHT:
            break
        # 绘制主射线
        rect_main = rotated_img.get_rect(center=(int(pos_main[0]), int(pos_main[1])))
        screen.blit(rotated_img, rect_main)
        # 绘制上方平行射线（偏移+64像素）
        pos_upper = (pos_main[0] + perp[0]*64, pos_main[1] + perp[1]*64)
        rect_upper = rotated_img.get_rect(center=(int(pos_upper[0]), int(pos_upper[1])))
        screen.blit(rotated_img, rect_upper)
        # 绘制下方平行射线（偏移-64像素）
        pos_lower = (pos_main[0] - perp[0]*64, pos_main[1] - perp[1]*64)
        rect_lower = rotated_img.get_rect(center=(int(pos_lower[0]), int(pos_lower[1])))
        screen.blit(rotated_img, rect_lower)

    """
    绘制 Master Spark 技能指示器：
      - 主射线从魔理沙中心向鼠标方向延伸（无限长）
      - 两条平行射线分别在主射线上下偏移 64 像素
    使用采样图"sample/skill/marisa/masterspark.png"，原始尺寸48×1，放大四倍为192×4。
    """
    import math
    if game_state.get("selected_skill") == 4 and game_state.get("aiming", False):
        # 获取魔理沙中心
        current_player = game_state["current_turn"]["active_player"]
        marisa_pos = map_layout["start_positions"][CONTROLLED_INDEX]
        marisa_center = (marisa_pos[0] * GRID_SIZE + GRID_SIZE // 2, marisa_pos[1] * GRID_SIZE + GRID_SIZE // 2)
        ...
        # 计算旋转角度：采样图默认正上
        angle = -math.degrees(math.atan2(direction[0], -direction[1]))
        # 加载并放大采样图：原始48×1放大4倍为192×4
        sample_img = load_scaled_image("sample/skill/marisa/masterspark.png", (192, 4))
        rotated_img = pygame.transform.rotate(sample_img, angle)
        # 计算垂直于射线的单位向量
        perp = (-direction[1], direction[0])
        
        # 定义一个足够长的射线长度（如直到地图边界）
        max_length = 10000
        # 以较大的步长（例如步长取采样宽度 192）铺贴即可保证连续
        step = 192  
        for d in range(0, max_length, step):
            # 主射线位置
            pos_main = (marisa_center[0] + direction[0]*d, marisa_center[1] + direction[1]*d)
            # 若超出屏幕则结束
            if pos_main[0] < 0 or pos_main[0] >= WINDOW_WIDTH or pos_main[1] < 0 or pos_main[1] >= WINDOW_HEIGHT:
...
def draw_masterspark_effect(screen):
    """
    当 Master Spark 状态激活时，绘制实际激光效果。
    绘制方式与指示器相似，但该效果在技能释放后持续显示2回合（每回合开始应用伤害）。
    """
    import math
    if "masterspark" not in game_state:
        return
    ...
    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
        if game_state.get("selected_skill") == 2:
            ...
            if character == "marisa":
                ...
                game_state["selected_skill"] = None
                return
    # --- Master Spark 输入处理 begin ---
    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
        if game_state.get("selected_skill") == 4:
            # 仅对魔理沙有效：获取当前操控玩家与角色
            current_player = game_state["current_turn"]["active_player"]
            character = game_state["players"][current_player]["character"]
            if character != "marisa":
                ...
                return
            # 检查灵力是否足够（固定消耗10）
            if PLAYER_STATS[current_player]["mana"] < 10:
                add_announcement("灵力不足，无法释放Master Spark")
                game_state["selected_skill"] = None
                return
            # 扣除10灵力
            PLAYER_STATS[current_player]["mana"] -= 10
            ...
            # 设置 Master Spark 状态，持续2回合
            game_state["masterspark"] = {
                "direction": direction,            # 单位向量
                "start_pos": marisa_center,          # 激光起点：魔理沙中心
                "activation_turn": game_state["current_turn"]["turn_number"],
                "remaining_rounds": 2                # 持续2回合
            }
            # 标记魔理沙在这2回合内无法移动
            game_state["masterspark_immobile"] = True
            
            # 计算 Master Spark 经过的所有网格（宽3格）
            game_state["masterspark_reveal"] = compute_masterspark_reveal()
            
            # 立即应用一次 Master Spark 伤害（当前回合）
            apply_masterspark_damage()
            
            add_announcement("魔理沙釋放了Master Spark")
            finish_turn("masterspark")
            game_state["selected_skill"] = None
            return
    # --- Master Spark 输入处理 end ---
...
def main():
    ...
    while running:
        ...
        update_vision_boost()
        update_masterspark()  # 更新 Master Spark 状态
        ...
        # 绘制 Master Spark：若状态激活则绘制实际激光效果，
        # 否则如果处于技能4瞄准阶段则绘制指示器（可选）
        if "masterspark" in game_state:
            draw_masterspark_effect(screen)
        elif game_state.get("selected_skill") == 4 and game_state.get("aiming", False):
            draw_masterspark_indicator(screen)
