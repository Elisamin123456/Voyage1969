...
def finish_turn(action_type, extra_info=""):
    ...
    char = P1_character if active == "P1" else P2_character
    if action_type == "move":
        msg = f"{char} 進行了移動"
...
SKILL_INFO = {
    ...
    (10, 9): ["靈力輸入框"],
    (11, 9): ["增加靈力"],
    (0, 9): ["建造", "消耗：n 靈力","效果：在n格半徑內", "建造生命值=n的牆體" ],
    (0, 10) :  ["偵察", "消耗：1 靈力","效果：顯示敵方目前所在座標", "當敵方在草叢時無效" ]
}
...
def draw_info_panel(screen, mouse_pos):
    ...
    if cell in map_layout["start_positions"]:
        ...
        hover_text = f"牆體生命值: {wall_health.get(cell, 0)}"
...
def draw_mist(screen):
    """
    根據當前視野半徑覆蓋迷霧：
      - 若存在視野提升效果（vision_boost），則使用該效果中的半徑；
      - 否則使用預設的 P1_vision_radius。
    對不在視野內的每個格子，繪製 mist.png 影像。
    """
...
def draw_vision_indicator(screen):
    ...
    if "vision_boost" in game_state:
        ...
        pos_y = (unit_cell[1] - 1) * GRID_SIZE  # 将指示器显示在机体正上方一格
...
def handle_input(event):
    ...
    if game_state.get("building", False):
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            center_cell = unit_cell  # 以玩家當前位置為中心
            ...
            if grid_x < 0 or grid_x >= MAP_WIDTH or grid_y < 0 or grid_y >= MAP_HEIGHT:
                ...
                if PLAYER_STATS["P1"]["mana"] >= game_state["current_mana_input"]:
                    ...
                    wall_health[target_cell] = game_state["current_mana_input"]
                game_state["building"] = False
